...
public class Backup {
    ...
    public static void getList(Player p, Player o) throws SQLException {
        ...
        Inventory inventory = Bukkit.createInventory(p, 27, ChatUtil.color("&7Backup'y gracza"));
        while (rs.next()) {
            String player = rs.getString("name");
            long time = rs.getLong("time");
            int ping = rs.getInt("ping");
            String killer = rs.getString("killer");
            ItemBuilder i = new ItemBuilder(Material.BOOK).setTitle(ChatUtil.color("ID: " + time));
            i.addLore(ChatUtil.color("&6Gracz: &c" + player));
            i.addLore(ChatUtil.color("&6Ping: &c" + ping));
            i.addLore(ChatUtil.color("&6Zabojca: &c" + killer));
            i.addLore(ChatUtil.color("&6Data utworzenia: &c" + DataUtil.getDate(time)));
